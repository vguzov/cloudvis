...
class VideoScroller:
    def __init__(self, video_path: Union[Path, str], scrolling_thresh: int = 100, cache_size: int = 100, target_fps=60., output_resolution=None):
        ...
        self.videoiter = iter(VideoReader(video_path, output_fps=self.target_fps, output_resolution=self.output_resolution))
    ...
    def reload_videoreader(self, frame_ind: int):
        self.videoiter = iter(
            VideoReader(self.video_path, start_frame=frame_ind, output_fps=self.target_fps, output_resolution=self.output_resolution))
